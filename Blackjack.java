 ...
 *
 * modified by: <Cynthia Chan>, <ccynthia@bu.edu>
 */
...
public class Blackjack {
    ...
    public static void printResult(Player dealer, Player user) {
        /*** implement this method for PS 10, Task 4 ***/
        if (dealer.getHandValue() > user.getHandValue() && user.getHandValue() < 21 
                || dealer.getHandValue()< 21) {
            System.out.println("You lose, " +user.getName() +".");
        } 
        else if (user.getHandValue() == dealer.getHandValue() && 
                 user.getHandValue() <= 21 && dealer.getHandValue()<=21) {
            ...
            System.out.println("Blackjack! Congrats, "+user.getName()+"!");
                     ...
                     && user.getHandValue() > dealer.getHandValue()) {
            System.out.println("You win, "+user.getName()+"!");
        } else if (dealer.getHandValue()>21 || user.getHandValue() < dealer.getHandValue()) {
            System.out.println("You win, " +user.getName() +"!");
        }
